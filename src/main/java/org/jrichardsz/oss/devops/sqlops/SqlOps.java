...
import org.jrichardsz.oss.common.ascii.TableAscciHelper;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.jrichardsz.oss.common.cli.ArgumentsHelper;
import org.jrichardsz.oss.common.database.DatabaseExecutor;
import org.jrichardsz.oss.common.exception.ExceptionHelper;
import org.jrichardsz.oss.common.file.ClassPathProperties;
import org.jrichardsz.oss.common.file.FileHelper;
import org.jrichardsz.oss.common.logger.LoggerHelper;
  ...
  public HashMap<String, Object> perform(String databaseHost, int databasePort, String databaseName,
    ...
    if (ClassPathProperties.hasProperty(engine + ".errorQueryFile"))
      if (databaseDbaUser != null && !databaseDbaPassword.isEmpty()) {
        ...
        beforeErrors = this.databaseHelper.executeSimpleScriptString(engine, databaseHost,
            databasePort, databaseName, databaseUser, databasePassword, sqlShowErrors);
    ...
    try {
      for (String scriptPath : queries) {
        ...
        ArrayList<?> scriptOutput = this.databaseHelper.executeSimpleScriptFile(engine,
            databaseHost, databasePort, databaseName, databaseUser, databasePassword,
            databaseDbaUser, databaseDbaPassword, currentScript);
        logger.info(
            String.format("script: %s , status: success", new Object[] {currentScriptSingleName}));
      ...
      if (ClassPathProperties.hasProperty(engine + ".errorQueryFile"))
        if (databaseDbaUser != null && !databaseDbaPassword.isEmpty()) {
          afterErrors = this.databaseHelper.executeSimpleScriptString(engine, databaseHost,
              databasePort, databaseName, databaseDbaUser, databaseDbaPassword, sqlShowErrors);
      ...
      String errorMessage =
          String.format("script: %s , status: error", new Object[] {currentScriptSingleName});
      ...
      if (executedQueries.size() < 1) {
        ...
        for (String executedScript : executedQueries) {
          ...
          try {
            scriptOutput = this.databaseHelper.executeSimpleScriptFile(engine, databaseHost,
            ...
            logger.error(String.format("rollback: %s , status: success",
                new Object[] {singleRollbackScriptName}));
          } catch (Exception ex) {
            logger.error("If a rollback fails, God, what more do you want from me?");
            logger.error(String.format("rollback: %s , status: error",
                new Object[] {singleRollbackScriptName}));
